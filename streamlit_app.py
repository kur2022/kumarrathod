...
def validate_csv_structure(df):
    required_columns = [
        'Stock Name', 'Price', 'Volume', 'Avg_Week_Volume',
        'Day_Low', 'Week_Low', 'Month_Low',
        'Day_High', 'Week_High', 'Month_High',
        'Signal Type'
    ]
...
def main():
    ...
    if response.status_code == 200:
        ...
        st.subheader("📝 Edit CSV Data Before Processing")
        df_csv = st.data_editor(df_csv, num_rows="dynamic", use_container_width=True)
    else:
        st.error("❌ Unable to load CSV data.")
        st.stop()

    # ✅ Download option
    st.download_button("💾 Download Edited CSV", df_csv.to_csv(index=False), "updated_data.csv")

    # ✅ Run Signal Engine
    if st.button("🚀 Run Signal Engine"):
        if not validate_csv_structure(df_csv):
            st.stop()
